...
export class ChatRoom {
  ...
  initDatabase() {
    this.sql.exec(`
      ...
      CREATE INDEX IF NOT EXISTS idx_pinned_channel ON pinned_messages(channel, pinned_at DESC);
    `);
  }
  ...
  createApp() {
    return ignite((app) => {
      ...
      app.post('/pin', async (c) => {
        try {
          const { messageId, channel, username } = await c.req.json();

          console.log('Pin request:', { messageId, channel, username });

          if (!messageId || !channel || !username) {
            return c.json(
              {
                error: 'Missing required fields: messageId, channel, username',
              },
              400,
            );
          }
          ...
          const messageCursor = this.sql.exec(
            `SELECT message_id, timestamp, username as name, message FROM messages WHERE message_id = ? AND channel = ?`,
            messageId,
            channel,
          );
          const messageRows = messageCursor.toArray();

          console.log('Found messages:', messageRows.length);

          if (messageRows.length === 0) {
            // Debug: Let's see what messages exist in this channel
            const debugCursor = this.sql.exec(
              `SELECT message_id, channel FROM messages WHERE message_id = ?`,
              messageId,
            );
            const debugRows = debugCursor.toArray();
            console.log('Message exists in other channels:', debugRows);
          ...
          const pinnedCursor = this.sql.exec(
            `SELECT id FROM pinned_messages WHERE message_id = ? AND channel = ?`,
            messageId,
            channel,
          );
          const pinnedRows = pinnedCursor.toArray();
          ...
          this.sql.exec(
            `INSERT INTO pinned_messages (message_id, channel, pinned_by, pinned_at) VALUES (?, ?, ?, ?)`,
            messageId,
            channel,
            username,
            pinnedAt,
          ...
          // Broadcast pin update to all clients
          this.broadcast({
            pinUpdate: {
              action: 'pin',
              messageId,
          ...
          return c.json({ success: true, pinnedAt });
        } catch (err) {
      ...
      // Unpin a message
      app.delete('/pin/:messageId', async (c) => {
        try {
          const messageId = c.req.param('messageId');
          ...
          const pinnedRows = cursor.toArray();

          if (pinnedRows.length === 0) {
            return c.json({ error: 'Message not pinned' }, 404);
          }

          // Unpin the message
          this.sql.exec(
            `DELETE FROM pinned_messages WHERE message_id = ? AND channel = ?`,
            messageId,
            channel,
          );

          // Broadcast unpin update to all clients
          this.broadcast({
            pinUpdate: {
              action: 'unpin',
              messageId,
      ...
      app.get('/channel/:channelName/pins', async (c) => {
        try {
          const channelName = c.req.param('channelName');
